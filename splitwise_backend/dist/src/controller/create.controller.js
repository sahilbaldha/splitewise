import { prisma } from "../lib/prisma.js";
// create a group
export const createGroup = async (req, res) => {
    const { groupName, usersIds } = req.body;
    const { userId } = req.user;
    try {
        const existingUsers = await prisma.user.findMany({
            where: {
                id: {
                    in: usersIds,
                },
            },
            select: {
                id: true,
            },
        });
        const arrayofIds = existingUsers.map((ele) => ele.id);
        const missingUserIds = usersIds.filter((ele) => !arrayofIds.includes(ele));
        if (missingUserIds.length > 0) {
            return res.status(201).json(missingUserIds);
        }
        const newGroupWithUsers = await prisma.group.create({
            data: {
                group_name: groupName,
                members: {
                    connect: usersIds.map((id) => ({ id })),
                },
                author_id: userId,
            },
            select: {
                members: {
                    select: {
                        id: true,
                        email: true,
                        name: true,
                    },
                },
            },
        });
        return res
            .status(201)
            .json({ message: "Group Create Sucessfully", data: newGroupWithUsers });
    }
    catch (error) {
        console.error("Failed to create group:", error);
        res.status(500).json({ error: "Something went wrong" });
    }
};
// create a expense
// const image = req.file as Express.Multer.File;
// const { userId } = req.user;
// if (!image) {
//   return res.status(400).json({ message: "Please upload an image" });
// }
// const imagePath = `${req.protocol}://${req.get("host")}/images/${image.filename}`;
// console.log(imagePath, name, category_id);
